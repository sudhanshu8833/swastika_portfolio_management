...
class run_strategy():
    ...
    def ltp_nifty_options(self, message,token_dict,dict_token):
        ...
        for i in range(len(position_opened)):
            try:
                position_opened[i].current_price=float(self.ltp_prices[position_opened[i].token])
    ...
    def close_all_positions(self,token_dict, dict_token,ws=22):
    ...
    def main(self,token_dict, dict_token,ws):
        ...
        if (float(self.ltp_prices['26000'])> self.parameters.T5 and self.parameters.T_now==4):
            ...
            self.close_all_positions(token_dict, dict_token,ws)
